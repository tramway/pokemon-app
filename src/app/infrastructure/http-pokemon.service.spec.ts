...
describe('HttpPokemonService', () => {
  ...
  it('should fetch list of pokemons and details', (done) => {
    ...
    ((req) => req.url.startsWith('https://pokeapi.co/api/v2/pokemon/1') && req.method === 'GET');
    bulbasaurDetailsRequest.flush(bulbasaurDetailsResponse());

    const pikachuDetailsRequest = httpTestingController.expectOne
    ((req) => req.url.startsWith('https://pokeapi.co/api/v2/pokemon/25') && req.method === 'GET');
    pikachuDetailsRequest.flush(pikachuDetailsResponse());
...
function bulbasaurDetailsResponse() {
  return {
    'abilities': [
      {
        'ability': {
